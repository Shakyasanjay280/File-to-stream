# webserver.py (FULL, UPDATED & FINAL CODE)
...
async def lifespan(app: FastAPI):
    ...
    # --- YEH HAI SABSE ZAROORI HISSA (THE MOST IMPORTANT PART) ---
    # Bot ko uske saare chats (groups/channels) ki list fetch karne ke liye force karo.
    # Isse STORAGE_CHANNEL uski memory (cache) mein aa jayega.
    # Iske baad "Peer ID Invalid" error kabhi nahi aayega.
    try:
        print("Caching all chats the bot is a member of... (This may take a moment)")
        async for _ in bot.get_dialogs():
            pass # Humein dialogs se kuch karna nahi, bas loop chalana hai taaki cache ban jaaye
        print("✅ Chat caching complete. Bot is now aware of all its channels.")
    except Exception as e:
        print(f"!!! FATAL ERROR: Could not get dialogs to build cache. Error: {e}")
    ...
    if bot.is_initialized:
        await bot.stop()
    # Baaki clients ko alag se stop karne ki zaroorat nahi, woh main process ke saath band ho jaate hain
    print("Bot stopped. Goodbye!")

# FastAPI app ko lifespan manager ke saath initialize karo
app = FastAPI(lifespan=lifespan)
templates = Jinja2Templates(directory="templates")

# Cache to store ByteStreamer instances for each client
class_cache = {}
...
def mask_filename(name: str) -> str:
    """ Obfuscates the filename to protect it. """
